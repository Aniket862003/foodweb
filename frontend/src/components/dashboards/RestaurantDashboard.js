...
const RestaurantDashboard = () => {
  ...
  const handleSaveProfile = async () => {
    try {
      const response = await axios.put(
        'https://foodweb-backend-g881.onrender.com/api/auth/update', 
        profileData, 
  ...
  const fetchOrders = async () => {
    ...
    try {
      const res = await axios.get(
        "https://foodweb-backend-g881.onrender.com/api/orders/restaurant-orders", 
        { headers: { Authorization: localStorage.getItem("token") } }
      );
      setOrders(res.data || []);
    } catch (err) {
  ...
  const fetchRestaurants = async () => {
    ...
    try {
      const res = await axios.get(
        "https://foodweb-backend-g881.onrender.com/api/restaurants/my-restaurants", 
        { headers: { Authorization: localStorage.getItem("token") } }
      );
      setRestaurants(res.data || []);
    } catch (err) {
      console.error("Restaurant fetch error:", err);
      alert(err.response?.data?.message || "Failed to fetch restaurants");
    } finally {
  ...
  const handleSubmit = async (e, endpoint, data, successCallback) => {
    ...
    try {
      const res = await axios.post(
        `https://foodweb-backend-g881.onrender.com/api/${endpoint}`,
        formData,
        {
          headers: { 
      ...
      console.error(`Error in ${endpoint} operation:`, error);
      alert(error.response?.data?.message || `Error in ${endpoint.split('/')[0]} operation`);
    }
  };
  ...
  const viewMenu = async (restaurantId) => {
    ...
    try {
      const res = await axios.get(
        `https://foodweb-backend-g881.onrender.com/api/restaurants/${restaurantId}/menu`
      );
      setSelectedRestaurant(restaurantId);
      setMenuItems(res.data || []);
    } catch (err) {
      console.error("Menu fetch error:", err);
      alert(err.response?.data?.message || "Failed to fetch menu");
    } finally {
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      await axios.put(
        `https://foodweb-backend-g881.onrender.com/api/orders/update-status/${orderId}`,
        { status: newStatus },
        { headers: { Authorization: localStorage.getItem("token") } }
      );
      fetchOrders();
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-lg relative animate-fadeIn">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <p className="text-gray-500">{profileData.phone || "No phone number"}</p>
          <p className="text-gray-500">{profileData.address || "No address"}</p>
            ...
            <div key={field}>
              ...
              {isEditing ? (
                ...
                <p className="mt-1 text-gray-800">
                  {profileData[field] || `No ${field} provided`}
                </p>
              )}
            </div>
          ))}
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full">
        ...
        <div className="space-y-4">
          <div>
            <h4 className="font-semibold">How do I manage my restaurants?</h4>
            <p className="text-gray-600">You can add and view your restaurants in the Restaurant Management section.</p>
          </div>
          <div>
            <h4 className="font-semibold">How do I update order status?</h4>
            <p className="text-gray-600">Click on the status buttons in the Order Management table to update order status.</p>
          </div>
        </div>
      </div>
    </div>
  ...
  const renderContactModal = () => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full">
        ...
        <div className="space-y-4">
          <p className="text-gray-600">Email: support@foodweb.com</p>
          <p className="text-gray-600">Phone: +1 (555) 123-4567</p>
          <p className="text-gray-600">Business Hours: 9AM - 5PM, Monday to Friday</p>
        </div>
      </div>
    </div>
  );
          ...
          {!selectedRestaurant ? (
                  ...
                  >
                    {['name', 'address', 'cuisine'].map(field => (
                      <input 
                        ...
                        onChange={(e) => handleInputChange(e, setRestaurantData)} 
                        required
            ...
            >
              ...
              <form 
                onSubmit={(e) => handleSubmit(
                  e, 
                  'restaurants/add-menu', 
                  { ...menuData, restaurantId: selectedRestaurant }, 
                  (data) => {
              ...
              >
                {['name', 'description', 'price'].map(field => (
                  <input 
                    ...
                    onChange={(e) => handleInputChange(e, setMenuData)}
